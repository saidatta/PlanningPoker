(function(window, angular, undefined) {
  ...
  angular.module("ATS.SharedServices").service('FirebaseService', ['$q', '$cookieStore', '$firebaseObject', function($q, $cookieStore, $firebaseObject) {
    ...
    this.newRoom = function(roomId, name) {
    ...
    this.newUser = function(roomId, uuid, leader, userName) {
      var user = $firebaseObject(firebase.child('room s').child(roomId).child('users').child(uuid));
      angular.extend(user, {
        leader: leader,
        userName: userName,
        vote: null,
